...
/*for save*/
	if($btn=="Save"){
		$query="insert into usersprofile (id,name,contact,address,city,picpath,idpath,idtype,idnumb)values('$userid','$fullname','$mob','$address','$city','$pic_name','$idpic_name','$idtype','$idnum')";
...
//echo $idpic_name;
		...
		$query="update usersprofile set id='$userid', name='$fullname', contact='$mob', address='$address', city='$city', picpath='$pic_name', idpath='$idpic_name',idtype='$idtype',idnumb='$idnum' where id='$userid'";
		mysqli_query($dbcon,$query);
		$msg=mysqli_error($dbcon);
		if($msg=="")
			header("location:provider-response.php?userid=".$userid."&msg=Record Updated Successfully!!");
		else
			echo $msg;
	}
?>
